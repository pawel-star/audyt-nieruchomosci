...
## 2.14
- **Naprawiony import odpowiedzi wtyczki, gdy nagłówek nie ma kodu pytania.** Wtyczka zwróciła `### Jaki grunt jest pod spodem?` bez `\`g_geo\``, a `znajdzKod()` szukał wyłącznie identyfikatora — więc cała odpowiedź, razem z notatką, przepadała bez śladu. Nowa funkcja `kodPoTresci()` dopasowuje pytanie po znormalizowanej treści nagłówka (bez wielkości liter, ogonków i interpunkcji), najpierw dokładnie, potem po zawieraniu, wybierając najdłuższe trafienie. Działa tylko w wierszach wyglądających na nagłówek (`###` albo `**pogrubienie**`), żeby nie łapać zdania ze środka notatki.
- **Naprawione ucinanie notatki w pół zdania.** Notatkę kończyła pusta linia oraz każdy wiersz zaczynający się od gwiazdki albo myślnika — czyli akapit `**Ustalenia:**` i zwykła lista wypunktowana. W realnej odpowiedzi wtyczki zapisywało się 392 znaki z 1225. Teraz notatka trwa do nowego pytania, wiersza opcji albo linii poziomej; pusta linia jest traktowana jak przerwa akapitowa, a pogrubienia są zdejmowane przy zapisie.
- Komunikaty błędu w obu oknach importu mówią, czego aplikacja szuka (kod w apostrofach albo treść pytania po `###`) i wskazują najczęstszą przyczynę: wklejony fragment bez nagłówka.
- Blok „TRYB PRACY” w promptach dostał dwie linijki: polecenie odpowiadania po polsku (wtyczka potrafiła zwrócić całe uzasadnienie po angielsku) oraz wymóg przepisania nagłówka razem z kodem w apostrofach.
- Sprawdzone: `node --check` oraz test na dokładnym tekście, który wrócił z wtyczki dla `g_geo` — punkt dostaje notatkę w całości i znacznik „sprawdzone, bez rozstrzygnięcia”, bez wybierania odpowiedzi, bo profilu otworu nie udało się odczytać. Do tego test negatywny: nagłówek niepasujący do żadnego pytania nie dopisuje niczego.
